...
public partial class PropertyEditorPage : ContentPage {
  ...
  void BindToView() {
    ...
    foreach (var p in _vm.OperatorInfo.GetImplementationProperties()) {
      var hbox =
          new HorizontalStackLayout { VerticalOptions = LayoutOptions.Center };
      ...
      if (p.PropertyType == typeof(int)) {
        ...
        stepper.ValueChanged += (sender, e) => label.Text =
            $"{p.Name}: {e.NewValue:000}";
  ...
  void BindToObject() {
    foreach (var m in _widgetMap) {
      if (m.Key.PropertyType == typeof(int)) {
        m.Key.SetValue(_vm.OperatorInfo.Instance,
                       Convert.ToInt32(((Stepper)m.Value).Value),
                       new object[0]);
      } else if (m.Key.PropertyType == typeof(bool)) {
        m.Key.SetValue(_vm.OperatorInfo.Instance, ((CheckBox)m.Value).IsChecked,
                       new object[0]);
      } else if (m.Key.PropertyType == typeof(float)) {
        m.Key.SetValue(_vm.OperatorInfo.Instance,
                       Convert.ToSingle(((Slider)m.Value).Value),
                       new object[0]);
      } else if (m.Key.PropertyType == typeof(double)) {
        m.Key.SetValue(_vm.OperatorInfo.Instance, ((Slider)m.Value).Value,
                       new object[0]);
      } else if (m.Key.PropertyType == typeof(TimeSpan)) {
        var hbox = (HorizontalStackLayout)m.Value;
        var seconds = ((Slider)hbox.Children[0]).Value;
        m.Key.SetValue(_vm.OperatorInfo.Instance, TimeSpan.FromSeconds(seconds),
                       new object[0]);
      }
    }
  }
