...
public class TspSampleController : SampleControllerBase {
  ...
  public override IView CreateConfigView() {
    ...
    generateButton.Clicked += (e, args) => { OnReconfigured(); };
    Grid.SetColumnSpan(generateButton, 2);

    var showIndexesLabel = new Label { Text = "Show indexes" };
    var showIndexes = new CheckBox { IsChecked = _showIndexes };
    showIndexes.CheckedChanged += (e, args) => { _showIndexes = args.Value; };
  ...
  public override ISelection CreateSelection() { return new EliteSelection(); }

  public override void ConfigGA(GeneticAlgorithm ga) {
    ga.TaskExecutor =
        new ParallelTaskExecutor { MinThreads = ga.Population.MinSize,
  ...
  /// Resets the sample.
  /// </summary>
  public override void Reset() { _bestChromosome = null; }
  ...
  public override void Draw(ICanvas canvas) {
    ...
    if (_bestChromosome != null) {
      ...
      for (int i = 0; i < genes.Length; i += 2) {
        ...
        if (i > 0) {
          var previousCity =
              _fitness.Cities[Convert.ToInt32(genes[i - 1].Value)];
      ...
      var lastCity =
          _fitness.Cities[Convert.ToInt32(genes[genes.Length - 1].Value)];
