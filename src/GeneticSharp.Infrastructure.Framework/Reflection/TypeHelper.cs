...
public static class TypeHelper {
  ...
  public static IList<Type> GetTypesByInterface<TInterface>() =>
      GetTypesByInterface(typeof(TInterface).Name);
  ...
  public static IList<Type> GetTypesByInterface(string interfaceName) {
    ...
    var selectedAssemblies = assemblies.Where(
        a => a.FullName.StartsWith("GeneticSharp.",
  ...
  CreateInstanceByName<TInterface>(string name,
                                   params object[] constructorArgs) {
    var crossoverType = GetTypeByName<TInterface>(name);

    try {
      return (TInterface)Activator.CreateInstance(crossoverType,
                                                  constructorArgs);
    } catch (MissingMethodException ex) {
      throw new ArgumentException(
          ("A {0}'s implementation with name '{1}' was found, but seems the " +
           "constructor args were invalid.").With(typeof(TInterface).Name,
                                                  name),
          nameof(constructorArgs), ex);
    }
  }
  ...
  /// <param name="implementationName">The TInterface's implementation
  /// name.</param> <typeparam name="TInterface">The interface.</typeparam>
  public static Type GetTypeByName<TInterface>(string implementationName) =>
      GetTypeByName(typeof(TInterface).Name, implementationName);

  /// <summary>
  /// Gets the TInterface's implementation with the specified name.
  /// </summary>
  /// <param name="interfaceName">The interfaces name</param>
  /// <param name="implementationName">The interface's implementation
  /// name.</param> <returns>The interfaces's implementation type.</returns>
  public static Type GetTypeByName(string interfaceName,
                                   string implementationName) {
    var implementationType =
        GetTypesByInterface(interfaceName)
            .FirstOrDefault(
                t => t.Name.Equals(implementationName,
                                   StringComparison.OrdinalIgnoreCase) ||
                     GetDisplayNameAttribute(t).DisplayName.Equals(
                         implementationName,
                         StringComparison.OrdinalIgnoreCase));

    if (implementationType == null) {
      throw new ArgumentException(
          "There is no {0} implementation with name '{1}'.".With(
              interfaceName, implementationName),
          nameof(implementationName));
    }
  ...
  GetDisplayNameAttribute(MemberInfo member) {
    var attribute = member.GetCustomAttributes(false).FirstOrDefault(
        a => a is DisplayNameAttribute);

    if (attribute == null) {
      throw new InvalidOperationException(
          "The member '{0}' has no DisplayNameAttribute.".With(member.Name));
    }
